...
*Background. This analyses the **current production scheduler** (`src/scheduling.rs`) to motivate
the MVCC + capacity-aware redesign. It does not describe the WIP `multistep_scheduler`. See
[README.md](README.md).*

## Overview

The current scheduler assigns chunks to workers using **consistent-hashing-with-bounded-loads**. Assignments are recalculated from scratch each run, but consistent hashing keeps most assignments stable unless inputs change. This document describes when and why chunks move between workers, and the operational cost of each trigger. The reshuffle cost catalogued here — especially the replication-factor cliffs and the worker-departure failures — is exactly what the redesign aims to smooth out.
...
*Test: `scenario_reliability_flip` in `src/tests/scenarios.rs`*
...
*Test: `scenario_new_chunks_added` in `src/tests/scenarios.rs`*
...
*Test: `scenario_capacity_changes` in `src/tests/scenarios.rs`*
...
- **R** — replication factor for a given chunk weight. Computed by `calc_replication_factors()` as `clamp(round(weight * K), min_replication, num_workers)`, where K is a multiplier chosen so that total replicated data fills the available capacity. Each chunk gets R independent virtual replicas, each hashed to a different ring.
...
The `assign_chunks()` function enforces strict capacity bounds. When a worker's home position on
the ring is full, the replica walks to the next ring position with room. If **no** worker has
room, `assign_chunks` **panics** — the legacy scheduler has no graceful fallback at the placement
step. (`NotEnoughCapacity` is returned earlier, by the `calc_replication_factors` pre-check when
`total_size × min_replication > total_capacity`; it is *not* what placement-step overflow
produces.)
