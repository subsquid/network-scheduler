...
/// `(chunk, worker)` copies the storage expired during a cycle's GC. Lets the metric score a
/// same-worker drain→refetch — a real download a step-boundary diff misses.
