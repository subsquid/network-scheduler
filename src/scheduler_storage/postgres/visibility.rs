...
pub(super) async fn replay_confirmed_diffs(
    ...
    //
    // One statement: `final` has two consumers (the upsert CTE and the delete), so it materializes
    // once — the diff window's scan + chunk_pk sort (14M rows under confirmation lag) used to run
    // twice as two statements. The upsert and delete touch disjoint chunk_pk sets (a chunk's final
    // holder set is either empty or not), so folding them changes nothing else. `rows_affected`
    // reports only the outer DELETE; the upsert count is no longer separable.
