//! Scenario measurements: concrete churn numbers for documentation.
//!
//! These tests print before/after reshuffling statistics for representative
//! cluster changes (worker join/leave, new chunks, reliability flips, capacity
//! changes). They assert only loose bounds; their main purpose is the printed
//! output used to document scheduler behaviour.
...
/// Fixed worker capacity used across all scenario tests for consistency.
...
/// Fixed chunk size used across all scenario tests.
const SCENARIO_CHUNK_SIZE: ByteSize = ByteSize::mib(100);

/// Default worker-set size used across scenario tests.
const SCENARIO_WORKERS: u16 = 2_000;

/// Default dataset size (chunk count) used across scenario tests.
const SCENARIO_CHUNKS: u32 = 500_000;

/// Generate input with fixed 100MB chunk sizes for scenario tests.
...
fn measure_reshuffle(
    ...
    removed_percentages.sort_by(|a, b| a.partial_cmp(b).unwrap());
    added_percantages.sort_by(|a, b| a.partial_cmp(b).unwrap());
...
fn scenario_worker_joins() {
    ...
    // Expected weight -> replication factor for each input, identical before and
    // after the join. Regression guard against scheduling-algorithm changes;
    // deterministic because 500k fixed-size chunks make the weight split converge.
...
fn scenario_worker_leaves() {
    ...
    // Expected weight -> replication factor for each input, identical before and
    // after one worker leaves. Regression guard against scheduling-algorithm
    // changes; deterministic because 500k fixed-size chunks make the split converge.
    let expected_weights: [BTreeMap<u16, u16>; 2] = [
        btreemap! { 1 => 23 },
        btreemap! { 1 => 2, 6 => 13, 24 => 54 },
    ];
...
fn scenario_many_workers_leave() {
    // Edge case: many workers leaving, forcing replication factors down to the minimum
    ...
    // Compute how many workers we need as a minimum for min_replication=22
    // Required capacity = 22 * total_size
    // Effective per-worker capacity = 600 GB * 0.95 = 570 GB
    // Min workers = ceil(22 * total_size / 570 GB)
    ...
    // Run baseline with all 2,000 workers, then measure S3 download relative to baseline
...
fn scenario_new_chunks_added() {
    ...
    for &(initial, added, weights) in &[
        ...
        // Build per-chunk ownership: chunk_index -> set of workers
        let mut owners_before: BTreeMap<u32, BTreeSet<PeerId>> = BTreeMap::new();
        for (worker, chunks) in &a1.worker_chunks {
            for &ci in chunks {
                owners_before.entry(ci).or_default().insert(*worker);
            }
        }
        let mut owners_after: BTreeMap<u32, BTreeSet<PeerId>> = BTreeMap::new();
        for (worker, chunks) in &a2.worker_chunks {
            for &ci in chunks {
                owners_after.entry(ci).or_default().insert(*worker);
            }
        }

        // Count existing chunks that changed ownership
        let mut existing_changed = 0u32;
        let mut existing_unchanged = 0u32;
        for ci in 0..initial {
            let before = owners_before.get(&ci);
            let after = owners_after.get(&ci);
            if before != after {
                existing_changed += 1;
            } else {
...
fn scenario_reliability_flip() {
    ...
    for &(n_workers, flipping, n_chunks) in &[
        ...
        // Step 1: A -> B. Compute S3 download per group.
        // "Added" bytes = new chunks a worker must download from S3.
        ...
        // Step 2: B -> C. Compute S3 download per group.
        ...
        for (worker, chunks_c) in &state_c.worker_chunks {
            let chunks_b_set: BTreeSet<_> = state_b
                ...
                .map(|v| v.iter().copied().collect())
                .unwrap_or_default();
            // But returning workers may still have their State A data locally!
            // They only need to download chunks in C that they don't have from EITHER A or B.
            let chunks_c_set: BTreeSet<_> = chunks_c.iter().copied().collect();
            if flipping_ids.contains(worker) {
                // Returning worker has data from both State A (original) and State B (offline assignment).
                let chunks_a_set: BTreeSet<_> = state_a
                    .worker_chunks
                    .get(worker)
                    .map(|v| v.iter().copied().collect())
                ...
                // Stable worker has data from State B. Needs to download chunks in C not in B.
...
fn scenario_capacity_changes() {
    ...
    for &(new_cap_gb, label) in &[
        ...
        // Calculate S3 download (added bytes per worker)
